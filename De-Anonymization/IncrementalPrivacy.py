...
with open("Output1.txt", "a") as text_file:
    for count in range(20):
        model = Sequential()
        model.add(Dense(20, input_dim=184, activation='relu',kernel_regularizer=l2(0.03)))
        model.add(Dense(classes, activation='softmax'))
        model.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['accuracy'])
        model.summary()


        model_output = model.fit(X_train,y_train,epochs=100,steps_per_epoch=(2579//10),verbose=1,validation_data=(X_val,y_val),validation_steps=1105//10)
        ...
        print("here")
        text_file.write("60% Privacy!, Accuracy is! %f \n" % (acc))